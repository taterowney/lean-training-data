...
def isOriginal (i : Info) : Bool :=
  ...
  | some stx => match stx.getHeadInfo, stx.getTailInfo with
...
/- Does this tactic actually appear in the source code or was it created by a macro?
Parser-produced `node`s carry `SourceInfo.none` themselves; `.original` info only lives on
leaf tokens, so we check the head and tail tokens. Requiring both guards against macro
expansions that splice user-written antiquotations at one end. -/
def isOriginal (info : TacticInfo) : Bool :=
  match info.stx.getHeadInfo, info.stx.getTailInfo with
  | .original .., .original .. => true
  | _, _ => false
...
ignoring structuring tactics (e.g. `by`, `;`, multiline tactics, parenthesized tactics) and those generated by macros.
-/
def tactics (t : InfoTree) : List (TacticInfo × ContextInfo) :=
  t.findTacticNodes.map (fun ⟨i, ctx, _⟩ => ⟨i, ctx⟩)
    |>.filter fun i => i.1.isSubstantive && i.1.isOriginal
